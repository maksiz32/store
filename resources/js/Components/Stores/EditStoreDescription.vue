...
const emit = defineEmits(['eventSuccess', 'closeEditDescription']);
let dialog = ref(props.show);

const submit = () => {
    form.put(route('store.update', props.store.store_id), {
            onSuccess: () => {
                emit('eventSuccess', `Store <strong>${props.store.name_store}</strong> was updated`);
                form.reset();
                emit('closeEditDescription');
                dialog.value = false;
            },
            onError: (error) => {
                form.errors = error;
    ...
    <teleport to="body">
        <div v-if="dialog" class="fixed inset-0 overflow-y-auto px-4 py-6 sm:px-0 z-50" scroll-region>
            <form class="overflow-auto" @submit.prevent="submit">
                <v-card>
                    <v-card-title>
                        <span class="text-h5">Change Store Description</span>
                    </v-card-title>
                    <v-card-text>
                        <v-container>
                    ...
                    <v-card-actions>
                        ...
                        <v-btn
                            color="blue-darken-1"
                            variant="text"
                            @click="emit('closeEditDescription')"
                        >
                            Close
                        </v-btn>
                        <v-btn
